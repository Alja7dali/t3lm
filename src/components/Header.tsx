...
export const Header = () => {
  ...
  const trailingElements = links
    ...
    .map((l) => {
      if ("style" in l) {
        if (l.style === "fill") {
          return <Link className="p-1 px-3 text-center w-56 md:w-auto text-white bg-[#363ED0] hover:bg-blue-600 border-2 border-[#363ED0] hover:border-blue-600 rounded-lg" to={l.url}>
            {lang === "ltr" ? l.en : l.ar}
          </Link>
        } else {
          return <Link className="p-1 px-3 text-center w-56 md:w-auto text-[#363ED0] hover:text-blue-600 border-2 border-[#363ED0] hover:border-blue-600 rounded-lg" to={l.url}>
            {lang === "ltr" ? l.en : l.ar}
          </Link>
        }
      } else {
  ...
  return (
    <header dir={lang} className="fixed top-0 bg-gray-100 w-full z-50">
      ...
      <nav className="max-w-7xl flex flex-col md:flex-row justify-between items-center gap-4 py-6 px-10 text-base mx-auto">
        <a href="/">
          <img src={logo} className="p-1 h-8 mx-auto"/>
        </a>
        <div ref={navRef} className="w-full hidden md:flex flex-col md:flex-row justify-between items-center gap-4">
          {leadingElements}
